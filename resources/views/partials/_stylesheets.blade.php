<!-- mobile menu css -->
<link rel="stylesheet" href="{{ secure_asset('css/meanmenu.min.css') }}" />
<!-- animate css -->
<link rel="stylesheet" href="{{ secure_asset('css/animate.css') }}" />
<!-- nivo slider css -->
<link rel="stylesheet" href="{{ secure_asset('css/nivo-slider.css') }}" />
<!-- owl carousel css -->
<link rel="stylesheet" href="{{ secure_asset('css/owl.carousel.min.css') }}" />
<!-- slick css -->
<link rel="stylesheet" href="{{ secure_asset('css/slick.css') }}" />
<!-- price slider css -->
<link rel="stylesheet" href="{{ secure_asset('css/jquery-ui.min.css') }}" />
<!-- fontawesome css -->
<link rel="stylesheet" href="{{ secure_asset('css/font-awesome.min.css') }}" />
<!-- fancybox css -->
<link rel="stylesheet" href="{{ secure_asset('css/jquery.fancybox.css') }}" />
<!-- bootstrap css -->
<link rel=" stylesheet" href="{{ secure_asset('css/bootstrap.min.css') }}" />
<!-- default css  -->
<link rel="stylesheet" href="{{ secure_asset('css/default.css') }}" />
<!-- style css -->
<link rel="stylesheet" href="{{ secure_asset('css/style.css') }}" />
<!-- responsive css -->
<link rel="stylesheet" href="{{ secure_asset('css/responsive.css') }}" />
<!-- modernizr js -->
<script src="{{ secure_asset('js/vendor/modernizr-2.8.3.min.js') }}"></script>
{{-- Algolia instant search --}}
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/instantsearch.css@7.3.1/themes/reset-min.css">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/instantsearch.css@7.3.1/themes/algolia-min.css">
